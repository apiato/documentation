...
:::info
`HashIdTrait` was being used in the Controller and Request class.
:::
...
You can now configure subdomains in the `bootstrap/app.php` file,
using the `Apiato` configuration class `withRouting` method. TODO add doc link

## Version Prefix
The `apiato.api.enable_version_prefix` configuration option has been removed.

You can now configure version prefixes in the `bootstrap/app.php` file,
using the `Apiato` configuration class `withRouting` method. TODO add doc link
