...
You can include relationships for complex data structures using the `include` query parameter.

The `include` parameter can be used on any endpoint
that has [relationships defined](../../main-components/transformers.md#model-relationships) in its transformer.

For instance, if you have a `Car` model with `Driver` and `Type` relationships,
and you want to retrieve all `Car` objects, you can include the `Driver` relationship by calling:

```
api.apiato.test/v1/cars?include=driver
```
...
```
api.apiato.test/v1/cars?include=driver,type
```
...
For example, if a `Driver` has a relationship with an `Address` object,
you can request nested includes using dot notation:

```
api.apiato.test/v1/cars?include=driver,driver.address
// or the shorter version
api.apiato.test/v1/cars?include=driver.address
```

This will include the `Driver` relationship with the `Address` nested under it:
...
Note that the `Address` relationship must also be defined on the `Driver` transformer.
