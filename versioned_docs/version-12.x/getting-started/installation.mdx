...
### Subdomain and API Version Prefix {#subdomain-and-api-version-prefix}

By default, Apiato uses `api` as a subdomain for all endpoints and adds only the API version as a prefix,
...
assuming you are using the default [Subdomain and API Version Prefix](#subdomain-and-api-version-prefix) configuration,
you should be able to access the following URLs:

- [http://apiato.test](http://apiato.test)
-> You should see an HTML page displaying the text `Apiato` in the center.

- [http://api.apiato.test](http://api.apiato.test)
...
http://api.apiato.test/ You should see a JSON response with the message: `"Welcome to apiato."`
http://api.apiato.test/v1 You should see a JSON response with the message: `"Welcome to Apiato (API V1)."`
...
* [Customized Laravel Components](customized-laravel-components.md)
* [Framework Features](../framework-features/index.md)
